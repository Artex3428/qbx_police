...
local function normalCuffPlayer(ped, targetPed, targetPlayer, cuffType, slot)
    ...
    TriggerServerEvent("police:syncNormalCuff", targetPlayer, angle, cuffType, slot)
    Wait(100)
...
local function agressiveCuffPlayer(ped, targetPed, targetPlayer, cuffType, slot)
    ...
    TriggerServerEvent("police:syncAgressiveCuff", targetPlayer, "back", cuffType, slot, heading)
    Wait(100)
...
local function uncuffPed(ped, cuffType)
    ...
    TriggerServerEvent("police:uncuffPed", serverId, cuffType)
...
lib.callback.register("police:syncAgressiveCuff", function(angle, cuffType, heading)
    local ped = cache.ped
    local coords = GetEntityCoords(ped)
    local dict = "mp_arrest_paired"
    local escaped = false
...
RegisterNetEvent("police:syncNormalCuff", function(angle, cuffType)
    setCuffed(true, angle, cuffType)
end)

RegisterNetEvent("police:uncuffPed", function()
    setCuffed(false)
end)
...
AddEventHandler("police:unziptie", function()
...
RegisterNetEvent('police:client:CuffPlayer', function()
    if IsPedRagdoll(cache.ped) or cache.vehicle then
        return
    end
...
RegisterNetEvent('police:client:ResetRestraints', function()
    if handsUpStatus then
        toggleHandsUp(false)
    end
    ...
    state:set('isCuffed', false, true)
    state:set('cuffType', false, true)
    state:set('isEscorted', false, true)
    state:set('handsUp', false, true)
    state:set('gettingCuffed', false, true)
    state:set('isCuffing', false, true)
    state:set('blockHandsUp', false, true)

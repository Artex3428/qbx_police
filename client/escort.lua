...
function StopEscortPlayer(serverId, setIntoVeh, setIntoSeat)
    TriggerServerEvent('police:setPlayerEscort', serverId, false, setIntoVeh, setIntoSeat)
...
local function escortPlayer(ped, id, onlyAnim, exitVeh)
    ...
    TriggerServerEvent('police:setPlayerEscort', GetPlayerServerId(id), not IsEntityAttachedToEntity(ped, cache.ped), false, false, exitVeh)
end
...
exports.ox_target:addGlobalVehicle({
    {
        name = "police:vehicleEscortExitPassenger",
    ...
    {
        name = "police:vehicleEscortExitDriver",
        icon = "fa-solid fa-right-from-bracket",
        label = locale("take_out_from_vehicle"),
        distance = 1.5,
        bones = {"seat_pside_r"},
...
AddStateBagChangeHandler('isEscorted', ('player:%s'):format(cache.serverId), function(_, _, value)
    isEscorted = value
    IsEscorted = value and true or false
...
RegisterNetEvent('police:client:EscortPlayer', function()
    local coords = GetEntityCoords(cache.ped)
    local player, targetPed = lib.getClosestPlayer(coords, 1.5, false)

    if not player or not targetPed then
